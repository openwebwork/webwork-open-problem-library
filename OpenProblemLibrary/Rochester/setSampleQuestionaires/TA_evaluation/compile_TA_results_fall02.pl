...
##
## REVISION HISTORY
## A. Pizer (Rochester) June 2000
## A. Pizer (Rochester) November 2000
## A. Pizer (Rochester) May 2001
...
## into 4 files.  The orginal files have the format described at the end of this file.

## The required inputs are the html source of the questionnaire, a file containing all the
## email responses concatenated into one file, and a base file name [e.g. TA_evalation_fall02]
## (without an extension) for the four output files [the four output files will be e.g.
## TA_evalation_fall02.html, TA_evalation_fall02_summary.html, TA_evalation_fall02.xls, and
## TA_evalation_fall02.txt].

## TA_evalation_fall02_summary.html is a small file containing a summary of the responses of question 5
## which is the overall evaluation and for question C "Would you take a recitation with this TA again?"

## TA_evalation_fall02.html is a large file containing detailed information on each TA along with all
## student written comments. The comments are ordered in the approximate order of from better to worst.

## TA_evalation_fall02.xls is a large file suitable for importation to a spreadsheet program such as Excel.
## It contains all numerical data.

## TA_evalation_fall02.txt is a large file suitable for importation to a spreadsheet program such as Excel.
## It contains all written comments.  The numerical and written data are keyed to each other.
...
(i.e. compile_TA_results_spring01.pl  questionnaire.html  raw_data.txt  spring01_TA_results\n";
...
#    next unless ($line =~ /Mode=([^&]*)/);
# input looks like
#<input type="hidden" name="Mode" value="HTML_dpng" />
    next unless ($line =~ m|<input type="hidden" name="Mode" value="(.*)"|);
    $mode = $1;
#print "mode is $mode\n" ;					#uncomment for debugging
    last;
...
# <H3>MTH 140A TA's </H3>
#
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="1" >Jenna Arndt				W	3:25-4:15		Hylan 105
#<P>
#
#
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="2" >Karen Bobrow			R	4:50-5:40		Hylan 102
#<P>
#
#<P>
 #<H3>MTH 141 TA's </H3>
#
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="3" >Jessica Bunch			M	11:00-11:50		Hylan 1104
#<P>
#
#
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="4" >Carmen Lemoine			F	11:00-11:50		Hylan 201
#<P>
#
#
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="5" >Carmen Lemoine			M	6:15-7:05PM		Meliora 204
#<P>
...
else {  ## text or tth mode or HTML_dpng
...
# <H3>MTH 140A TA's </H3>
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="1" >Jenna Arndt				W	3:25-4:15		Hylan 105 <BR>
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="2" >Karen Bobrow			R	4:50-5:40		Hylan 102 <BR>
#
# <H3>MTH 141 TA's </H3>
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="3" >Jessica Bunch			M	11:00-11:50		Hylan 1104 <BR>
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="4" >Carmen Lemoine			F	11:00-11:50		Hylan 201 <BR>
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="5" >Carmen Lemoine			M	6:15-7:05PM		Meliora 204 <BR>
#<INPUT TYPE=RADIO NAME="AnSwEr2" VALUE="6" >Rong Liu					W	12:00-12:50		Hylan 101 <BR>
#
#print "$line/n";
...
while (<RAWDATA>) {
    ...
    if ( $in =~ /^\s*(\d+)\-Problem\-\d*\-Question\-([^\:]+):/  ) {
        ...
        if ( $question_number =~/6/ ) {
            ...
            $essay_response{$TA_number}{$label} .= $in;
        }
        else {
...
foreach $TA_number (@TA_number_array) {
	...
	while(<QUESTIONNAIRE>){
	    ...
	    if ($line =~ /^<!DOCTYPE HTML PUBLIC/) {         ## skip initial stuff

NEXTLINE:	        while(<QUESTIONNAIRE>) {
	                $line =$_;
	                next NEXTLINE unless $line =~ /^<!-- BEGIN_PG_PROBLEM_TEXT -->/ ;
	                last NEXTLINE if $line =~  /^<!-- BEGIN_PG_PROBLEM_TEXT -->/ ;
	            }
            }

	    last if $line =~ /^<TEXTAREA/;
	    if($line =~ /^Dear Students/) {
	    ...
	    next if $line =~ /^Dear Students/;
	    next if $line =~ /^We would very/;
	    next if $line =~ /^Please answer/;

		print OUT $line unless $line =~ /^\s*<HR>/;
    	if($line =~ /^<h2>([A-Z])\./){$question = $1};
    	if($line =~ /^(\d+)\./){$numeric = $1};      ## questions 1-5
    	if($line =~ /^<h2>\s*(\d+)\./){$numeric = $1}; ## question 6
    	next unless ($line =~ /^\s*<HR>/ or $line =~ /^\d+\./ or $line =~ /^<h2>\s*\d+\./);
...
LINE:	        while(<QUESTIONNAIRE>) {
	                $line =$_;
	                next LINE unless $line =~ /^\s*<HR>/ ;
	                last LINE if $line =~ /^\s*<HR>/ ;
	            }
            }
		}
    	...
    	else {  ## print out essay responses
    	    print  OUT '<p><p>';
            foreach  $key2 (@questions) {
                foreach  $key (@sorted_psvn_keys) {
                    if (defined $essay_response{$TA_number}{"$key - $key2"})
