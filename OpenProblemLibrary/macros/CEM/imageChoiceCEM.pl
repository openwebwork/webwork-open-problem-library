...
sub _imageChoiceCEM_init {}; # don't reload this file

######################################################################
#
#  Create a match list where the answers are images
#
#  Usage:  $ml = new_image_match_listCEM(options);
#
...
sub img_print_a {
  ...
  my %options = (
     ...
     separation => 10, vseparation => 0, link => 0, @{$self->{ImageOptions}});
  ...
  $out = BeginTable(tex_spacing => "5pt", tex_border => "5pt", border=> $border);
  while ($image = shift) {
    push(@images, $image);
